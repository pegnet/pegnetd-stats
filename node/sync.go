...
func (d *Pegnetd) SyncBlock(ctx context.Context, tx *sql.Tx, stats *pegnet.Stats, height uint32) error {
	...
	if height >= TransactionConversionActivation {
		...
		if gradedBlock != nil && 0 < len(gradedBlock.Winners()) {
			...
			if err = d.ApplyTransactionBatchesInHolding(ctx, tx, stats, height, rates); err != nil {
				return err
			}
		}
	...
	//    These funds will be available for transactions and conversions executed in the next block
	if gradedBlock != nil {
		if err := d.ApplyGradedOPRBlock(tx, stats, gradedBlock, dblock.Timestamp); err != nil {
			return err
		}
	}
	return nil
...
func (d *Pegnetd) ApplyTransactionBatchesInHolding(ctx context.Context, sqlTx *sql.Tx, stats *pegnet.Stats, currentHeight uint32, rates map[fat2.PTicker]uint64) error {
	...
	for i := height; i < currentHeight; i++ {
		...
		if currentHeight >= PegnetConversionLimitActivation && currentHeight < V4OPRUpdate {
			...
			err = d.recordPegnetRequests(sqlTx, stats, pegConversions, rates, currentHeight, bank, int32(currentHeight-1))
			if err != nil {
				return err
			}
	...
	if currentHeight >= V4OPRUpdate {
		...
		err = d.recordPegnetRequests(sqlTx, stats, pegConversions, rates, currentHeight, uint64(bentry.BankAmount), int32(currentHeight))
		if err != nil {
			return err
		}
	}
...
func (d *Pegnetd) ApplyTransactionBlock(sqlTx *sql.Tx, stats *pegnet.Stats, eblock *factom.EBlock) error {
	for blockorder, entry := range eblock.Entries {
		...
		if err = d.applyTransactionBatch(sqlTx, txBatch, stats, nil, eblock.Height); err != nil &&
...
func (d *Pegnetd) recordPegnetRequests(sqlTx *sql.Tx, stats *pegnet.Stats, txBatchs []*fat2.TransactionBatch, rates map[fat2.PTicker]uint64, currentHeight uint32, bank uint64, bankHeight int32) error {
...
// If an error is returned, the sql.Tx should be rolled back by the caller.
func (d *Pegnetd) ApplyGradedOPRBlock(tx *sql.Tx, stats *pegnet.Stats, gradedBlock grader.GradedBlock, timestamp time.Time) error {
	winners := gradedBlock.Winners()
	for i := range winners {
